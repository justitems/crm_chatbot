...
/*
 * Read class extends the class Numbers to make it lighter
 * 
 * @author Scrisoft
 * @package CRM
 * @since 0.0.8.5
...
class Read {
    ...
    /**
     * The public method crm_chatbot_the_numbers gets the numbers list by page
     * 
     ...
     */
    public function crm_chatbot_the_numbers($params) {
        ...
        // Set where
        $where = array(
            'crm_chatbot_numbers.user_id' => $this->CI->user_id
        );      
        ...
        // Set like
        $like = isset($params['key'])?array('number' => trim(str_replace('!_', '_', $this->CI->db->escape_like_str($params['key'])))):array();

        // Set join
        $join = array(
            array(
                'table' => 'crm_chatbot_websites',
                'condition' => 'crm_chatbot_numbers.website_id=crm_chatbot_websites.website_id',
                'join_from' => 'LEFT'
            )
        );
        ...
        // Set limit
        $limit = array(
            'order_by' =>  array('crm_chatbot_numbers.created', 'DESC'),
        );
        ...
        if ( md_the_cache('crm_chatbot_user_' . $this->CI->user_id . '_numbers_' . $parameters_string) ) {

            // Set the cache
            $the_numbers = md_the_cache('crm_chatbot_user_' . $this->CI->user_id . '_numbers_' . $parameters_string);
            ...
            $the_numbers = $this->CI->base_model->the_data_where(
                'crm_chatbot_numbers',
                'crm_chatbot_numbers.*, crm_chatbot_websites.domain',
                $where,
                $where_in,
                $like,
                $join,
            ...
            // Save cache
            md_create_cache('crm_chatbot_user_' . $this->CI->user_id . '_numbers_' . $parameters_string, $the_numbers);

            // Set saved cronology
            update_crm_cache_cronology_for_user($this->CI->user_id, 'crm_chatbot_numbers_list', 'crm_chatbot_user_' . $this->CI->user_id . '_numbers_' . $parameters_string);

        }

        // Verify if the numbers exists
        if ( $the_numbers ) {

            // Verify if the cache exists for this query
            if ( md_the_cache('crm_chatbot_user_' . $this->CI->user_id . '_load_total_numbers_' . $parameters_string) ) {

                // Get total numbers
                $the_total = md_the_cache('crm_chatbot_user_' . $this->CI->user_id . '_load_total_numbers_' . $parameters_string);
                ...
                $the_total = $this->CI->base_model->the_data_where(
                    'crm_chatbot_numbers',
                    'COUNT(crm_chatbot_numbers.number_id) AS total',
                    $where,
                    $where_in,
                    $like,
                    $join
                ...
                // Save cache
                md_create_cache('crm_chatbot_user_' . $this->CI->user_id . '_load_total_numbers_' . $parameters_string, $the_total);

                // Set saved cronology
                update_crm_cache_cronology_for_user($this->CI->user_id, 'crm_chatbot_numbers_list', 'crm_chatbot_user_' . $this->CI->user_id . '_load_total_numbers_' . $parameters_string);
