<?php
/**
 * Numbers Read Helper
 *
 * This file read contains the methods
 * to read the numbers
 *
 * @author Scrisoft
 * @package CRM
 * @author   Scrisoft <asksyn@gmail.com>
 * @license  https://github.com/scrisoft/crm_chatbot/blob/main/license CRM Chatbot License
 * @link     https://www.midrub.com/
 */

// Define the page namespace
namespace CmsBase\User\Apps\Collection\Crm_chatbot\Helpers\Parts\Numbers;

// Constants
defined('BASEPATH') OR exit('No direct script access allowed');

/*
 * Read class extends the class Numbers to make it lighter
 * 
 * @author Scrisoft
 * @package CRM
 * @since 0.0.8.5
*/
class Read {
    
    /**
     * Class variables
     *
     * @since 0.0.8.5
     */
    protected $CI;

    /**
     * Initialise the Class
     *
     * @since 0.0.8.5
     */
    public function __construct() {
        
        // Get CodeIgniter object instance
        $this->CI =& get_instance();
        
    }

    //-----------------------------------------------------
    // Ajax's methods for the Numbers
    //-----------------------------------------------------

    /**
     * The public method crm_chatbot_the_numbers gets the numbers list by page
     * 
     * @param array $params contains the parameters
     * 
     * @since 0.0.8.5
     * 
     * @return array with response
     */
    public function crm_chatbot_the_numbers($params) {

        // Verify if the team's member has permissions
        if ( !md_the_team_role_permission('crm_chatbot') ) {

            // Prepare the false response
            return array(
                'success' => FALSE,
                'message' => $this->CI->lang->line('crm_chatbot_no_numbers_were_found')
            );         

        }

        // Get team's member
        $member = $this->CI->session->userdata('member')?the_crm_current_team_member():array();

        // Set where
        $where = array(
            'crm_chatbot_numbers.user_id' => md_the_user_id()
        );      

        // Set where in
        $where_in = array();

        // Verify if member exists
        if ( $this->CI->session->userdata( 'member' ) ) {

            // Verify if member's role exists
            if ( isset($member['role_id']) ) {

                // Set team role
                $params['team_role'] = $member['role_id'];   

                // Get the websites
                $the_websites_list = $this->CI->base_model->the_data_where(
                    'crm_chatbot_websites',
                    '*',
                    array(
                        'user_id' => md_the_user_id()
                    )

                );

                // Verify if websites exists
                if ( $the_websites_list ) {

                    // Set websites container
                    $websites = array();     
                    
                    // List the websites
                    foreach ( $the_websites_list as $the_website ) {

                        // Verify if the website is allowed
                        if ( !the_crm_team_roles_multioptions_list_item(md_the_user_id(),  $member['role_id'], 'crm_chatbot_allowed_websites', $the_website['website_id']) ) {
                            continue;
                        }

                        // Add website to the list
                        $websites[] = $the_website['website_id'];

                    } 
                    
                    // Verify if $websites is not empty
                    if ( $websites ) {

                        // Set team's member to params
                        $params['team_member'] = 1;

                        // Set where in
                        $where_in = array('crm_chatbot_websites.website_id', $websites);

                    } else {

                        // Prepare the false response
                        return array(
                            'success' => FALSE,
                            'message' => $this->CI->lang->line('crm_chatbot_no_numbers_were_found')
                        );  

                    }

                }

            }

        }

        // Set like
        $like = isset($params['key'])?array('number' => trim(str_replace('!_', '_', $this->CI->db->escape_like_str($params['key'])))):array();

        // Set join
        $join = array(
            array(
                'table' => 'crm_chatbot_websites',
                'condition' => 'crm_chatbot_numbers.website_id=crm_chatbot_websites.website_id',
                'join_from' => 'LEFT'
            )
        );

        // Set limit
        $limit = array(
            'order_by' =>  array('crm_chatbot_numbers.created', 'DESC'),
        );

        // Verify if start exists
        if ( isset($params['page']) ) {

            // Set the start
            $limit['start'] = $params['page'] * 10;

            // Set the limit
            $limit['limit'] = $params['limit'];

        }

        // Get parameters string
        $parameters_string = $this->generate_string($params);

        // Verify if the cache exists for this query
        if ( md_the_cache('crm_chatbot_user_' . md_the_user_id() . '_numbers_' . $parameters_string) ) {

            // Set the cache
            $the_numbers = md_the_cache('crm_chatbot_user_' . md_the_user_id() . '_numbers_' . $parameters_string);

        } else {

            // Get the numbers numbers
            $the_numbers = $this->CI->base_model->the_data_where(
                'crm_chatbot_numbers',
                'crm_chatbot_numbers.*, crm_chatbot_websites.domain',
                $where,
                $where_in,
                $like,
                $join,
                $limit
            );

            // Save cache
            md_create_cache('crm_chatbot_user_' . md_the_user_id() . '_numbers_' . $parameters_string, $the_numbers);

            // Set saved cronology
            update_crm_cache_cronology_for_user(md_the_user_id(), 'crm_chatbot_numbers_list', 'crm_chatbot_user_' . md_the_user_id() . '_numbers_' . $parameters_string);

        }

        // Verify if the numbers exists
        if ( $the_numbers ) {

            // Verify if the cache exists for this query
            if ( md_the_cache('crm_chatbot_user_' . md_the_user_id() . '_load_total_numbers_' . $parameters_string) ) {

                // Get total numbers
                $the_total = md_the_cache('crm_chatbot_user_' . md_the_user_id() . '_load_total_numbers_' . $parameters_string);

            } else {

                // Get the total numbers
                $the_total = $this->CI->base_model->the_data_where(
                    'crm_chatbot_numbers',
                    'COUNT(crm_chatbot_numbers.number_id) AS total',
                    $where,
                    $where_in,
                    $like,
                    $join
                );

                // Save cache
                md_create_cache('crm_chatbot_user_' . md_the_user_id() . '_load_total_numbers_' . $parameters_string, $the_total);

                // Set saved cronology
                update_crm_cache_cronology_for_user(md_the_user_id(), 'crm_chatbot_numbers_list', 'crm_chatbot_user_' . md_the_user_id() . '_load_total_numbers_' . $parameters_string);

            }

            // Prepare success response
            return array(
                'success' => TRUE,
                'numbers' => $the_numbers,
                'total' => $the_total[0]['total']
            );

        }

    }

    //-----------------------------------------------------
    // Quick Helpers for the Numbers
    //-----------------------------------------------------

    /**
     * The protected method generate_string generates a string for cache file
     * 
     * @param array $args contains the parameters
     * 
     * @since 0.0.8.5
     * 
     * @return string
     */
    protected function generate_string($args) {

        // Create and return string
        return str_replace(' ', '_', implode('_', $args) );
        
    }

}

/* End of file read.php */